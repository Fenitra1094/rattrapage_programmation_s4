...
@Component({
  selector: 'app-prix-vente-oeufs',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './prix-vente-oeufs.component.html',
  styleUrls: ['../shared.css']
})
export class PrixVenteOeufsComponent implements OnInit {
  items: any[] = [];
  races: any[] = [];
  showModal = false;
  editing = false;
  ...
  load() {
    this.api.getPrixVenteOeufs().subscribe(data => {
      this.items = data;
      this.cdr.detectChanges();
    });
  }
  ...
  save() {
    ...
    const obs = this.editing
      ? this.api.updatePrixVenteOeuf(this.form.id, payload)
      : this.api.createPrixVenteOeuf(payload);
    obs.subscribe(() => { this.showModal = false; this.load(); });
  }

  delete(id: number) {
    if (confirm('Supprimer ce prix ?')) {
      this.api.deletePrixVenteOeuf(id).subscribe(() => this.load());
    }
  }
