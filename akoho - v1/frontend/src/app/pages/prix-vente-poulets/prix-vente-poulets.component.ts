...
@Component({
  selector: 'app-prix-vente-poulets',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './prix-vente-poulets.component.html',
  styleUrls: ['../shared.css']
})
export class PrixVentePouletsComponent implements OnInit {
  items: any[] = [];
  races: any[] = [];
  showModal = false;
  editing = false;
  ...
  load() {
    this.api.getPrixVentePoulets().subscribe(data => {
      this.items = data;
      this.cdr.detectChanges();
    });
  }
  ...
  save() {
    ...
    const obs = this.editing
      ? this.api.updatePrixVentePoulet(this.form.id, payload)
      : this.api.createPrixVentePoulet(payload);
    obs.subscribe(() => { this.showModal = false; this.load(); });
  }

  delete(id: number) {
    if (confirm('Supprimer ce prix ?')) {
      this.api.deletePrixVentePoulet(id).subscribe(() => this.load());
    }
  }
