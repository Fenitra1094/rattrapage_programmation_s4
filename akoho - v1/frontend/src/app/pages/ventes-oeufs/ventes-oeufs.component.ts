...
@Component({
  selector: 'app-ventes-oeufs',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './ventes-oeufs.component.html',
  styleUrls: ['../shared.css']
})
export class VentesOeufsComponent implements OnInit {
  items: any[] = [];
  clients: any[] = [];
  lots: any[] = [];
  showModal = false;
  ...
  load() {
    this.api.getVentesOeufs().subscribe(data => {
      this.items = data;
      this.cdr.detectChanges();
    });
  }
  ...
  save() {
    this.api.createVenteOeuf(this.form).subscribe(() => {
  ...
  delete(id: number) {
    if (confirm('Supprimer cette vente et tous ses détails ?')) {
      this.api.deleteVenteOeuf(id).subscribe(() => this.load());
    }
  }
}
